...
const CreateDailysController = async (req: Request, res: Response) => {
        const { title, hour, description, date, final_date, patient_id } = req.body
        const dailys = await CreateDailysService({ title, hour, description, date, final_date, patient_id })
